...
export class QuoteService {
  ...
  calculateFees(amount: number, provider: string, targetCountry = "NG") {
    // Frais simulés
    ...
    const fixedFee = 5;
    const aggregatorFee =
      provider === "Flutterwave" ? amount * 0.01 : amount * 0.005;

    return {
      transfer: transferFee,
      conversion: conversionFee,
      total:
        transferFee + conversionFee + percentFee + fixedFee + aggregatorFee,
      breakdown: {
        percentFee,
        fixedFee,
        aggregatorFee,
