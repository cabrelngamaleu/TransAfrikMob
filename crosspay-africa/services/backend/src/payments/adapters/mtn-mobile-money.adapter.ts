...
export class MtnMobileMoneyAdapter implements PaymentAdapter {
  ...
  async initiate(
    ...
    try {
      this.logger.log(
        `Initiating MTN Mobile Money payment for ${request.amount} ${request.currency}`
      );
      ...
      this.logger.error(
        "Erreur lors de l'initiation du paiement MTN Mobile Money",
        error
      );
      return {
        success: false,
  ...
  async verify(
    ...
    try {
      this.logger.log(
        `Vérification du paiement MTN Mobile Money: ${request.transactionId}`
      );
      ...
      this.logger.error(
        "Erreur lors de la vérification du paiement MTN Mobile Money",
        error
      );
      return {
        success: false,
    ...
    return {
      data: {
        ...
        providerReference:
          ...
          `MTN-REF-${Math.floor(Math.random() * 1000000)}`,
