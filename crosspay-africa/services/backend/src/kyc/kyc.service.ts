...
export class KycService {
  ...
  async create(createKycVerificationDto: CreateKycVerificationDto): Promise<KycVerification> {
    // Vérifier si l'utilisateur existe
    const user = await this.usersService.findOne(createKycVerificationDto.userId);
    if (!user) {
      throw new NotFoundException(`User with ID ${createKycVerificationDto.userId} not found`);
    }
    ...
    if (existingVerification) {
      throw new BadRequestException('User already has a pending KYC verification');
    }

    const kycVerification = new KycVerification();
    kycVerification.verificationAmount = createKycVerificationDto.verificationAmount;
  ...
  async update(id: string, updateKycVerificationDto: UpdateKycVerificationDto): Promise<KycVerification> {
    ...
    const updatedVerification = Object.assign(kycVerification, updateKycVerificationDto);
  ...
  async approveVerification(id: string, adminId: string): Promise<KycVerification> {
  ...
  async rejectVerification(id: string, adminId: string, rejectionReason: string): Promise<KycVerification> {
    if (!rejectionReason) {
